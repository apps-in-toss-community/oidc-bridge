...
src/
  ...
  storage/
    ...
    pg.ts            # Postgres (default)
    sqlite.ts        # SQLite (self-host fallback, single-app)
    migrations/
...
1. **Storage is an interface.** Postgres + SQLite are first-class. No
   Cloud SQL-specific SQL.
...
- **Rate limits**: per-IP sliding window on `/oidc/token`, per-app sliding
  window on all endpoints. In-memory per instance for initial release.
...
7 tables (Postgres migrations; SQLite mirror). `MasterKeyProvider`
interface + env + file providers. 6h cache. Rotation API at the provider
layer (no HTTP yet). Unit tests for HKDF + cache + provider conformance.
